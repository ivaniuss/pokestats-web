  ...
  return (
    <div className="mb-3 flex items-start gap-2">
      <details className="flex-1" ref={(el) => registerRef(name, el)}>
        ...
        {starterBuild.length > 0 && (
          <div className="mt-2 mb-2 rounded-lg border border-emerald-500/20 bg-emerald-500/10 px-3 py-2">
            <p className="text-xs font-semibold text-emerald-400 mb-1">Starter build — best for beginners</p>
            <div className="flex flex-wrap gap-2">
              {starterBuild.map((s) => (
                <span key={s.item} className="flex items-center gap-1 text-xs bg-slate-800 rounded-full px-2 py-1">
                  <ItemImg name={s.item} size={16} />
                  <span className="text-slate-200">{s.item}</span>
                  <span className="text-slate-500">avg {s.avg_rank.toFixed(2)}</span>
                </span>
              ))}
            </div>
          </div>
