...
export default function CompositionsPage() {
  ...
  const sorted = useMemo(() => data.sort((a, b) => a.mean_rank - b.mean_rank), [data])
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="text-slate-500 border-b border-slate-700">
            ...
            <th className="text-right py-2 pr-4">Winrate</th>
            <th className="text-right py-2 pr-4">Count</th>
            <th className="text-left py-2 pr-4">Synergies</th>
            <th className="text-left py-2">Core</th>
          </tr>
        </thead>
        <tbody>
          {sorted.slice(0, selectedId ? data.length : 20).map((c) => (
            <tr key={c.cluster_id}
            ...
            >
              ...
              <td className="py-2 pr-4 text-xs">{Object.entries(c.synergies || {}).map(([k, v]) => (
