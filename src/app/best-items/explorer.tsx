...
interface FilterBarProps {
  minCount: number
  onMinCount: (n: number) => void
  tierFilter: string
  tiers: string[]
  onTier: (t: string) => void
}

function FilterBar({ minCount, onMinCount, tierFilter, tiers, onTier }: FilterBarProps) {
  return <>
...
function Th({ sort, label, sortKey, sortAsc, onSort }: {
  sort: SortKey
  label: string
  sortKey: SortKey
  sortAsc: boolean
...
function PkmView({ selected, data, sortKey, sortAsc, onSort, minCount, tierFilter, tiers, onTierChange, onMinCountChange }: ViewProps & {
  data: ItemEntry[]
}) {
  const agg = useMemo(() => {
  ...
  const items = tierFilter === "ALL"
    ? agg
    : data.filter((i) => i.tier === tierFilter).map((i) => ({
      item: i.item, avg_rank: i.avg_rank, count: i.count, tiers: i.tier,
    }))

  const show = useMemo(() => {
    const f = items.filter((i) => i.count >= minCount && !isNaN(i.avg_rank))
    f.sort((a, b) => {
      const c = sortKey === "count" ? a.count - b.count : a.avg_rank - b.avg_rank
      return sortAsc ? c : -c
    })
    return f
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-4 flex-wrap">
        ...
        <FilterBar minCount={minCount} onMinCount={onMinCountChange} tierFilter={tierFilter} tiers={tiers} onTier={onTierChange} />
      </div>
...
function ItemView({ selected, data, sortKey, sortAsc, onSort, minCount, tierFilter, tiers, onTierChange, onMinCountChange }: ViewProps & {
  data: ItemAgg
}) {
  const tierData = tierFilter === "ALL"
    ? Object.entries(data.tiers)
    : Object.entries(data.tiers).filter(([t]) => t === tierFilter)

  const show = useMemo(() => {
    const f = tierData.filter(([, v]) => v.count >= minCount)
    f.sort((a, b) => {
          ...
          <tbody>{show.map(([tier, info]) => (
            <tr key={tier} className="border-b border-slate-800 hover:bg-slate-800/50">
              ...
              <td className="py-2 text-xs text-slate-400 flex items-center gap-1 flex-wrap">
                {info.pokemons.slice(0, 10).map((p) => (
...
export default function BestItemsExplorer({ recs }: { recs: Record<string, ItemEntry[]> }) {
  ...
  const itemMap = useMemo(() => {
    const inv: Record<string, ItemAgg> = {}
    for (const [pkm, entries] of Object.entries(recs)) {
  ...
  return (
    <div>
      ...
      {selected && isPkm && (
        <PkmView selected={selected} data={recs[selected] ?? []}
      ...
      {selected && !isPkm && itemMap[selected] && (
        <ItemView selected={selected} data={itemMap[selected]}
          sortKey={sortKey} sortAsc={sortAsc} onSort={toggleSort} minCount={minCount} tierFilter={tierFilter} tiers={tiers}
          onTierChange={setTierFilter} onMinCountChange={setMinCount} />
      )}
    </div>
